...
#### Repeat Customers
Counts the total number of customers who have made more than one booking.
```sql
...
Represents the total physical capacity of your fleet (sum of unique bus capacities).
```sql
// Name: Total Seating Capacity
SUM({ FIXED [Bus ID] : MIN([Capacity]) })
```

#### Seats Offered
Represents the total capacity offered across all scheduled journeys (cumulative sum).
```sql
// Name: Seats Offered
SUM([Capacity])
```

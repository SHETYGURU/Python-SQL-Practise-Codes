...
# ==========================================
# UNIT TESTS FOR ETL PIPELINE (PYTEST)
# ==========================================
# These tests validate the etl_pipeline cleaning, consistency, and formatting logic.
# Run these tests using: python -m pytest etl_pipeline.py

def test_transform_and_validate_dates():
    """Test date format standardization & invalid date removal"""
    bookings = pd.DataFrame({
        'Booking_ID': [1, 2, 3],
        'Customer_ID': [101, 101, 101],
        'Bus_ID': [201, 201, 201],
        'Route_ID': [301, 301, 301],
        'Booking_Date': ['2025-01-01', '12/31/2025', 'invalid-date'],
        'Travel_Date': ['2025-01-05', '2026-01-05', '2025-01-05'],
        'Seat_Number': [5, 10, 15],
        'Fare_Amount': [100.0, 200.0, 150.0],
    ...
    clean_b, clean_c, _, _ = transform_and_validate(bookings, customers, buses, routes)
    # The 'invalid-date' booking should be removed due to parsing failure
    assert len(clean_b) == 2
    assert '2025-01-01' in clean_b['Booking_Date'].values
    assert '2025-12-31' in clean_b['Booking_Date'].values

def test_transform_and_validate_deduplication():
    """Test duplicate booking removal by Booking_ID"""
    bookings = pd.DataFrame({
        'Booking_ID': [1, 1, 2],
        'Customer_ID': [101, 101, 101],
        'Bus_ID': [201, 201, 201],
        ...
        'Travel_Date': ['2025-01-05', '2025-01-05', '2025-01-05'],
        'Seat_Number': [5, 5, 10],
        'Fare_Amount': [100.0, 100.0, 200.0],
        'Booking_Status': ['confirmed', 'confirmed', 'confirmed']
    })
    customers = pd.DataFrame({
        'Customer_ID': [101],
    ...
    clean_b, _, _, _ = transform_and_validate(bookings, customers, buses, routes)
    # Deduplication should keep only the first unique occurrence
    assert len(clean_b) == 2

def test_transform_and_validate_nulls_and_outliers():
    """Test handling of null phone numbers, gender standardization, name capitalization, age outliers"""
    bookings = pd.DataFrame({
        'Booking_ID': [1],
        'Customer_ID': [101],
        'Bus_ID': [201],
    ...
    customers = pd.DataFrame({
        ...
        'Name': ['john doe', 'jane smith', 'bob builder'],
        'Email': ['john@example.com', 'jane@example.com', 'bob@example.com'],
        'Phone': [None, '1234567890', '9876543210'],
        'Gender': ['m', 'F', None],
        'Age': [30, 150, None]
    })
    buses = pd.DataFrame({
    ...
    _, clean_c, _, _ = transform_and_validate(bookings, customers, buses, routes)
    
    # Phone null filled with 'Unknown'
    assert clean_c.loc[clean_c['Customer_ID'] == 101, 'Phone'].values[0] == 'Unknown'
    # Name standard title cased
    assert clean_c.loc[clean_c['Customer_ID'] == 101, 'Name'].values[0] == 'John Doe'
    # Gender normalized
    assert clean_c.loc[clean_c['Customer_ID'] == 101, 'Gender'].values[0] == 'Male'
    assert clean_c.loc[clean_c['Customer_ID'] == 102, 'Gender'].values[0] == 'Female'
    assert clean_c.loc[clean_c['Customer_ID'] == 103, 'Gender'].values[0] == 'Female'
    # Age outlier (150) and null filled with median (which is 30 in this set)
    assert clean_c.loc[clean_c['Customer_ID'] == 102, 'Age'].values[0] == 30
    assert clean_c.loc[clean_c['Customer_ID'] == 103, 'Age'].values[0] == 30

def test_transform_and_validate_constraints():
    """Test negative fare amount and travel date before booking date"""
    bookings = pd.DataFrame({
        'Booking_ID': [1, 2, 3],
        'Customer_ID': [101, 101, 101],
        'Bus_ID': [201, 201, 201],
        'Route_ID': [301, 301, 301],
        'Booking_Date': ['2025-01-05', '2025-01-05', '2025-01-05'],
        'Travel_Date': ['2025-01-10', '2025-01-01', '2025-01-10'],
        'Seat_Number': [5, 10, 15],
        'Fare_Amount': [100.0, 200.0, -50.0],
        'Booking_Status': ['confirmed', 'confirmed', 'confirmed']
    })
    customers = pd.DataFrame({
        'Customer_ID': [101],
    ...
    clean_b, _, _, _ = transform_and_validate(bookings, customers, buses, routes)
    # Booking 2 (travel before booking) and Booking 3 (negative fare) should be removed
    assert len(clean_b) == 1
    assert clean_b['Booking_ID'].values[0] == 1

def test_transform_and_validate_capacity_check():
    """Test seat number exceeding bus capacity"""
    bookings = pd.DataFrame({
        'Booking_ID': [1, 2],
        'Customer_ID': [101, 101],
        'Bus_ID': [201, 201],
        ...
        'Travel_Date': ['2025-01-05', '2025-01-05'],
        'Seat_Number': [5, 45],
        'Fare_Amount': [100.0, 200.0],
    ...
    clean_b, _, _, _ = transform_and_validate(bookings, customers, buses, routes)
    # Booking 2 should be dropped because Seat_Number 45 exceeds Capacity 40
    assert len(clean_b) == 1
    assert clean_b['Booking_ID'].values[0] == 1
